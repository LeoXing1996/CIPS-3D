...
from tl2.proj.logger.textlogger import (
    summary_dict2txtfig,
...
from tl2.proj.pytorch.examples.dataset_stylegan3.dataset import (
...
def saved_models(
    model_dict, info_msg, G, G_ema, G_kwargs, fixed_z, img_size, saved_dir=None
):
...
def save_images(
    ...
    with torch.cuda.amp.autocast(use_amp_G):
        ...
        Gz = G(
            fixed_z, return_aux_img=True, forward_points=256**2, **copied_metadata
        )[0]
        save_image(
            Gz,
            f"{saved_dir}/0Gz.jpg",
    ...
    with torch.cuda.amp.autocast(use_amp_G):
        ...
        save_image(
            Gema_z_tilted,
            f"{saved_dir}/0Gz_tilted_ema.jpg",
            nrow=int(math.sqrt(bs)),
            normalize=True,
            scale_each=True,
        )
    ...
    for curriculum_step in curriculum.keys():
        if type(curriculum_step) == int:
            ...
            if curriculum_step_str in global_cfg.curriculum:
                curriculum[curriculum_step].update(
                    global_cfg.curriculum.get(curriculum_step_str).to_dict()
                )
        else:
            # update attrs
...
def build_optimizer(generator_ddp, discriminator_ddp):

    optimizer_G = torch.optim.Adam(
        params=[
            {"params": generator_ddp.parameters(), "initial_lr": global_cfg.gen_lr}
        ],
    ...
    optimizer_D = torch.optim.Adam(
        params=[
            {"params": discriminator_ddp.parameters(), "initial_lr": global_cfg.disc_lr}
        ],
...
def train(rank, world_size, opt):
    ...
    discriminator = build_model(
        cfg=global_cfg.D_cfg, kwargs_priority=True, diffaug=global_cfg.diffaug
    ).to(device)
    G_ema = copy.deepcopy(generator)
    ema_start_itr = global_cfg.get("ema_start_itr", 1000)
    ema_model = comm_model_utils.EMA(
        source=generator, target=G_ema, decay=0.999, start_itr=ema_start_itr
    )
    ...
    optimizer_G, optimizer_D = build_optimizer(
        generator_ddp=generator_ddp, discriminator_ddp=discriminator_ddp
    )
    ...
    if global_cfg.tl_resume or global_cfg.load_finetune:
        if global_cfg.tl_resume:
            ...
            torch_utils.load_models(
                save_dir=resume_dir, model_dict=model_dict, strict=False, rank=rank
            )
        elif global_cfg.load_finetune:
            moxing_utils.copy_data(
                rank=rank,
                global_cfg=global_cfg,
                datapath_obs=global_cfg.finetune_dir,
                datapath=global_cfg.finetune_dir,
            )
            finetune_model_dict = {
                "generator": generator_ddp.module,
                "G_ema": G_ema,
            ...
            torch_utils.load_models(
                save_dir=global_cfg.finetune_dir,
                model_dict=finetune_model_dict,
                strict=False,
                rank=rank,
            )
        else:
    ...
    """dataset """
    moxing_utils.copy_data(
        rank=rank, global_cfg=global_cfg, **global_cfg.obs_training_dataset
    )
    dataset = build_model(
        global_cfg.data_cfg, kwargs_priority=True, resize_resolution=global_cfg.img_size
    )
    ...
    for step in range(start_itr, global_cfg.total_iters):
        ...
        if nerf_noise_disable:
            ...
            nerf_noise = max(0, 1.0 - state_dict["step"] / 5000.0)
        ...
        with torch.cuda.amp.autocast(global_cfg.use_amp_D):
            ...
            with torch.no_grad():
                ...
                zs_list = generator.get_zs(
                    real_imgs.shape[0], batch_split=global_cfg.batch_split
                )
        ...
        for subset_z in zs_list:
            with torch.cuda.amp.autocast(global_cfg.use_amp_G):
                ...
                with torch.cuda.amp.autocast(global_cfg.use_amp_D):
                    g_preds, _, _ = discriminator_ddp(
                        gen_imgs.to(torch.float32), alpha=alpha, use_aux_disc=aux_reg
                    )
        ...
        # update ema
        ema_model.update(
            itr=state_dict["step"], source_dict=generator_ddp.module.state_dict()
        )
        ...
        if (
            ...
            summary_ddict["grad_points"]["grad_points"] = (
                grad_points if grad_points is not None else global_cfg.img_size**2
            )
            ...
            summary_ddict["train_aux_img"]["train_aux_img"] = int(
                global_cfg.train_aux_img
            )
            ...
            summary_str = tl2_utils.get_print_dict_str(
                summary_ddict, outdir=global_cfg.tl_outdir, suffix_str=pbar.get_string()
            )
            print(summary_str)
        ...
        if step == 0 or (step + 1) % global_cfg.eval_every == 0 or global_cfg.tl_debug:
            ...
            moxing_utils.copy_data(
                rank=rank, global_cfg=global_cfg, **global_cfg.obs_inception_v3
            )
            global_cfg.obs_inception_v3.disable = True
            if rank == 0:
                metric_dict = eval_fid(
                    real_dir=f"{global_cfg.tl_outdir}/exp/fid/real",
                    fake_dir=f"{global_cfg.tl_outdir}/exp/fid/fake",
                )
                logger.info(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--n_epochs", type=int, default=60000, help="number of epochs of training"
    )
    parser.add_argument(
        "--sample_interval",
        type=int,
