    ...
    def __init__(
        ...
        if use_style_fc:
            self.modulation = EqualLinear(
                style_dim, in_channel, bias_init=1, lr_mul=1., scale=scale)
        else:
        ...
        for name, param in local_state.items():
            ...
            if key in state_dict:
                ...
                try:
                    ...
                    try:
                        with torch.no_grad():
                            input_param = rearrange(
                                input_param, "1 out in 1 1 -> 1 in out")
                            param.copy_(input_param)
                    except Exception as ex:
...
class EqualLinear(nn.Module):
    ...
    def forward(self, input):
        ...
        """
        if self.norm_weight:
            demod = torch.rsqrt(self.weight.pow(
                2).sum([1, ], keepdim=True) + 1e-8)
