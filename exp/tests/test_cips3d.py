...
class Testing_ffhq_exp(unittest.TestCase):
    ...
    def test__build_generator(self, debug=True):
        ...
        cfg = TLCfgNode.load_yaml_with_command(
            'exp/cips3d/configs/ffhq_exp.yaml', command='_build_generator')
    ...
    def test__build_discriminator(self, debug=True):
        ...
        D = build_model(cfg.D_cfg, kwargs_priority=True,
                        diffaug=True).to(device)
    ...
    def test_render_video_web(self, debug=True):
        ...
        script = importlib.import_module(
            'tl2.proj.streamlit.scripts.run_web').__file__
        if debug:
            cmd_str = f"""
          python
            {script}
...
class Testing_ffhq_exp_v1(unittest.TestCase):
    ...
    def test__build_generator(self, debug=True):
        ...
        imgs = make_grid(imgs, nrow=int(np.sqrt(bs)),
