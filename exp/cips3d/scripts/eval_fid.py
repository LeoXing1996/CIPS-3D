...
def main():
    ...
    Checkpointer(G_ema).load_state_dict_from_file(
        global_cfg.network_pkl, rank=rank)

    moxing_utils.copy_data(rank=rank, global_cfg=global_cfg,
                           **global_cfg.obs_training_dataset)
    ...
    logger.info("=== End setup_evaluation.")
    ...
    moxing_utils.copy_data(rank=rank, global_cfg=global_cfg,
                           **global_cfg.obs_inception_v3)
    global_cfg.obs_inception_v3.disable = True
    if rank == 0:
        metric_dict = eval_fid(
            real_dir=real_dir, fake_dir=fake_dir, kid=global_cfg.kid)
