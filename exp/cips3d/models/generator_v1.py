...
from exp.comm.models import film_layer, inr_network, mod_conv_fc, nerf_network
from exp.dev.nerf_inr.models.generator_nerf_inr import \
    GeneratorNerfINR as GeneratorNerfINR_base
...
class NeRFNetwork_sigma(nn.Module):
    ...
    def __init__(self,
        ...
        # self.color_layer_sine = FiLMLayer(in_dim=_in_dim, out_dim=_out_dim, style_dim=style_dim, use_style_fc=True)
        # self.style_dim_dict[f'{name_prefix}_rgb'] = self.color_layer_sine.style_dim
        self.color_layer_sine = LinearSinAct(
            in_features=_in_dim, out_features=_out_dim)
...
class INRNetwork_Skip(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(hidden_layers):
            ...
            _layer = film_layer.FiLMLayer(
                in_dim=_in_dim, out_dim=_out_dim, style_dim=style_dim)
            self.network.append(_layer)
            self.style_dim_dict[f'{name_prefix}_w{i}_0'] = _layer.style_dim

            _layer = film_layer.FiLMLayer(
                in_dim=_out_dim, out_dim=_out_dim, style_dim=style_dim)
            self.network.append(_layer)
    ...
    def forward(self, input, style_dict, **kwargs):
        ...
        if global_cfg.tl_debug:
            VerboseModel.forward_verbose(
                self.tanh, inputs_args=(rgb,), name_prefix='tanh.')
        out = self.tanh(rgb)
        return out
...
class SinBlock(nn.Module):
    def __init__(self,
        ...
        #                                           # scale=None,
        #                                           )
        self.mod1 = mod_conv_fc.SinStyleMod(
            in_channel=in_dim,
            out_channel=out_dim,
            style_dim=style_dim,
            use_style_fc=True)
        ...
        #                                           # scale=None,
        #                                           )
        self.mod2 = mod_conv_fc.SinStyleMod(
            in_channel=out_dim,
            out_channel=out_dim,
            style_dim=style_dim,
            use_style_fc=True)
...
class CIPSNet(nn.Module):
    ...
    def __init__(self,
        ...
        for i, (name, channel) in enumerate(self.channels.items()):
            ...
            _to_rgb = ToRGB(in_dim=_out_dim,
                            dim_rgb=pre_rgb_dim, use_equal_fc=False)
...
class GeneratorNerfINR(GeneratorNerfINR_base):
    ...
    def whole_grad_forward(self,
            ...
            with torch.no_grad():
                ...
                for b in range(batch_size):
                    (
                        ...
                        pitch,
                        yaw,
                    ) = comm_utils.get_world_points_and_direction(
                        batch_size=1,
                        num_steps=num_steps,
                        img_size=img_size,
                        fov=fov,
        ...
        if global_cfg.tl_debug:
            VerboseModel.forward_verbose(
                self.filters, inputs_args=(inr_img,), name_prefix="filters.")
    ...
    def part_grad_forward(self,
        ...
        inr_img = comm_utils.scatter_points(
            idx_grad=idx_grad,
        ...
        if return_aux_img:
            aux_img = comm_utils.scatter_points(
                idx_grad=idx_grad,
    ...
    def points_forward(self,
        ...
        if idx_grad is not None:
            transformed_points = comm_utils.gather_points(
            ...
            transformed_ray_origins = comm_utils.gather_points(
                points=transformed_ray_origins, idx_grad=idx_grad)
            transformed_ray_directions = comm_utils.gather_points(
        ...
        if hierarchical_sample:
            ...
            all_z_vals = torch.cat(
                [fine_z_vals, z_vals], dim=-2)  # (b, n, s, 1)
    ...
    def forward_camera_pos_and_lookup(self,
        ...
        if global_cfg.tl_debug:
            VerboseModel.forward_verbose(
                self.mapping_network_nerf,
                inputs_args=(zs['z_nerf'],),
                submodels=['base_net'],
                name_prefix='mapping_nerf.')
            VerboseModel.forward_verbose(
                self.mapping_network_inr,
...
class GeneratorNerfINR_freeze_NeRF(GeneratorNerfINR):
    ...
    def load_nerf_ema(self, G_ema):
        ...
        ret = self.mapping_network_nerf.load_state_dict(
            G_ema.mapping_network_nerf.state_dict())
        ret = self.aux_to_rbg.load_state_dict(G_ema.aux_to_rbg.state_dict())

        ret = self.mapping_network_inr.load_state_dict(
            G_ema.mapping_network_inr.state_dict())
        ret = self.nerf_rgb_mapping.load_state_dict(
    ...
    def mapping_network(self,
        ...
        with torch.no_grad():
            style_dict.update(self.mapping_network_nerf(z_nerf))
            style_dict.update(self.mapping_network_inr(z_inr))
            style_dict['nerf_rgb'] = self.nerf_rgb_mapping(
                style_dict['nerf_rgb'])
