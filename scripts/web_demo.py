...
class CIPS_3D_Demo(object):
    ...
    def model_interpolation(self,
        ...
        target_network_pkl = st_utils.selectbox(
            'target_network_pkl', cfg.target_network_pkl)
        model_pkl_input = st_utils.text_input('model_pkl', "", sidebar=False)

        swapped_layers = st_utils.parse_list_from_st_text_input(
            'swapped_layers', cfg.swapped_layers)
        gamma_target = st_utils.number_input(
            'gamma_target', cfg.gamma_target, sidebar=True)

        num_steps = st_utils.number_input(
            'num_steps', cfg.num_steps, sidebar=True)
        image_size = st_utils.number_input(
            'image_size', cfg.image_size, sidebar=True)
        ...
        num_frames = st_utils.number_input(
            'num_frames', cfg.num_frames, sidebar=True)

        num_samples_translate = st_utils.number_input(
            'num_samples_translate', cfg.num_samples_translate, sidebar=True)
        translate_dist = st_utils.number_input(
            'translate_dist', 0.04, sidebar=True)

        fov = st_utils.number_input('fov', cfg.fov, sidebar=True)
        max_fov = st_utils.number_input('max_fov', cfg.max_fov, sidebar=True)
        alpha_pi_div = st_utils.number_input(
            'alpha_pi_div', cfg.alpha_pi_div, sidebar=True)

        # seed
        seed = st_utils.get_seed(cfg.seeds_gallery)

        # trajectory
        trajectory_mode = st_utils.selectbox(
            'trajectory_mode', cfg.trajectory_mode, sidebar=True)
        forward_points = st_utils.number_input(
            'forward_points', cfg.forward_points, sidebar=True)
        ...
        if trajectory_mode == 'circle_near_far':
            ...
            xyz, lookup, yaws, pitchs = comm_utils.get_yaw_camera_pos_and_lookup(
                num_samples=num_frames, )
            xyz = torch.from_numpy(xyz).to(device)
            lookup = torch.from_numpy(lookup).to(device)
            fov_list = [fov] * len(xyz)
        ...
        video_f = cv2_utils.ImageioVideoWriter(
            f"{outdir}/{output_name}", fps=fps)
        ...
        with torch.no_grad():
            for idx in tqdm.tqdm(range(len(xyz))):
                ...
                merged_frame = pil_utils.merge_image_pil(
                    [frame_pil, frame_target_pil, frame_swapped_pil], nrow=2, )
...
def main(outdir,
    ...
    command_cfg = TLCfgNode.load_yaml_with_command(
        cfg_filename=cfg_file, command=command)

    logger = get_logger(
        filename=f"{outdir}/log.txt", logger_names=['st'], stream=False)
    logger.info(f"command_cfg:\n {command_cfg.dump()}")

    st_model = CIPS_3D_Demo()

    mode = st_utils.selectbox(
        label='mode', options=command_cfg.mode, sidebar=True)
...
def build_args():
    ...
    argparser_utils.add_argument_str(
        parser, name='outdir', default='results/model_interpolation')
    argparser_utils.add_argument_str(
        parser, name='cfg_file', default='configs/web_demo.yaml')
    argparser_utils.add_argument_str(
        parser, name='command', default='model_interpolation')
